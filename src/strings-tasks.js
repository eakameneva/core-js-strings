...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, indx) + str.slice(indx + value.length);
}
...
function reverseWords(str) {
  ...
  return reversedSentence + reversedWord;
}
...
function getCardId(value) {
  const masti = ['♣', '♦', '♥', '♠'];
  const hightValues = ['J', 'Q', 'K'];
  const cardAmountInRow = 52 / 4;
  const rowIndex = masti.findIndex((mast) => value.includes(mast));
  const highIndex = hightValues.findIndex((highValue) =>
    value.includes(highValue)
  );
  const valueWithoutMast = value.slice(0, -1);
  let numericValue;

  if (value.includes('A')) {
    numericValue = 1;
  } else if (highIndex >= 0) {
    numericValue = 10 + highIndex + 1;
  } else {
    numericValue = +valueWithoutMast;
  }

  return numericValue - 1 + cardAmountInRow * rowIndex;
}
